...
from dataset import MaskGuidedDataset, load_labels, load_viz_volumes
...
def get_probs(model, records, device, n_slices, nch=2):
    ds = MaskGuidedDataset(records, augment=False, n_slices=n_slices, n_ch_per_slice=nch)
    lbls, probs, pids = [], [], []
    model.eval()
    with torch.no_grad():
        for i in range(len(ds)):
            img, mask, _, lbl, pid = ds[i]
            p = torch.softmax(
...
class GradCAMPerSlice:
    ...
    def __call__(self, tensor, mask, class_idx=1):
        self.model.eval()
        self.model.zero_grad()
        x = tensor.unsqueeze(0).detach().requires_grad_(True)
        out = self.model(x, mask.unsqueeze(0))
...
def main(args):
    ...
    if args.gradcam:
        ...
        test_ds = MaskGuidedDataset(test_r, augment=False, n_slices=args.n_slices, n_ch_per_slice=nch)
        gradcam = GradCAMPerSlice(model, nch)
        for i, (pid, lbl, prob) in enumerate(zip(pids, test_lbl, test_prob)):
            tensor, mask, _, _, _ = test_ds[i]
            heatmaps = gradcam(tensor.to(device), mask.to(device))
            true  = 'csPCa' if lbl==1 else 'ciPCa'
            pred  = 'csPCa' if prob>=0.5 else 'ciPCa'
            ...
            for zi in display_slices:
                ...
                axes[0].set_title(f'T2W (ROI, slice {zi})'); axes[0].axis('off')
