"""
Training script for slice-wise MIL-max csPCa classification.
- Model: shared MedViT (3ch, no inflation) processes each slice independently
- MIL aggregation: max over per-slice csPCa logit → patient-level score
- Loss: BCEWithLogitsLoss on patient-level logit
- Differential LR: backbone=lr_backbone, proj_head=lr_head
"""
import argparse
import json
import os
...
def make_optimizer(model, lr_backbone, lr_head, weight_decay):
    ...
    print(f"  Optimizer: backbone LR={lr_backbone}, proj_head LR={lr_head}")
...
def eval_auc(model, loader, device):
    ...
    with torch.no_grad():
        for imgs, labels, _ in loader:
            ...
            prob = torch.sigmoid(patient_logit).cpu().numpy()
...
def train_model(train_records, val_records, args, device, nw):
    ...
    train_ds = SliceWiseDataset(train_records, augment=not args.aug_strong and not args.aug_scale,
        aug_scale=args.aug_scale,
                                aug_strong=args.aug_strong, aug_t2w_only=args.aug_t2w_only,
                                no_hflip=not args.hflip, gland_z_center=args.gland_z_center,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for imgs, lbls, _ in train_loader:
            imgs = imgs.to(device)         # [B, N, 3, H, W]
            lbls = lbls.float().to(device) # [B]
            optimizer.zero_grad()
        ...
        if val_auc > best_auc:
            best_auc, best_epoch, patience = val_auc, epoch, 0
...
def evaluate_test(ckpt_path, test_records, args, device, nw):
    test_ds     = SliceWiseDataset(test_records, augment=False, n_slices=args.n_slices)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cspca-weight', type=float, default=0.0,
                        help='Multiplier on auto pos_weight (n_neg/n_pos). 0=auto.')
