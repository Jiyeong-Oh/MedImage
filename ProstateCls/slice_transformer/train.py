"""
Training script for PI-CAI csPCa classification — slice Transformer variant.

Differential LR:
  - model.extractor.*  (pretrained MedViT backbone) → lr_backbone
  - everything else    (CLS, pos_embed, transformer, head) → lr_head
"""
import argparse
import json
import os
...
def make_optimizer(model, lr_backbone, lr_head, weight_decay):
    ...
    print(f"  Optimizer: backbone LR={lr_backbone}, transformer+head LR={lr_head}")
...
def train_model(train_records, val_records, args, device, nw):
    ...
    model = build_model(
        num_classes=2, pretrained=True, n_slices=args.n_slices,
        nhead=args.nhead, num_layers=args.num_layers,
        dim_feedforward=args.dim_feedforward, tf_dropout=args.tf_dropout,
    ...
    class_weights = compute_class_weights(
        np.array([r[1] for r in train_records]), device, args.cspca_weight)
...
def evaluate_test(ckpt_path, test_records, args, device, nw):
    ...
    model = build_model(
        num_classes=2, pretrained=False, n_slices=args.n_slices,
        nhead=args.nhead, num_layers=args.num_layers,
        dim_feedforward=args.dim_feedforward, tf_dropout=args.tf_dropout,
        head_depth=args.head_depth, backbone=args.backbone,
        pooling=args.pooling,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr-factor',       type=float, default=0.5,
                        help='ReduceLROnPlateau factor (LR multiplied by this on plateau)')
    ...
    parser.add_argument('--batch-size',      type=int,   default=4,
                        help='Keep small (4) — backbone processes B*32 images per step')
    ...
    parser.add_argument('--output-dir',      type=str,   default='./output/slice_tf')
