"""
Mask-guided dataset with gland bounding-box crop (mask_bbox).

Crop strategy: gland 2D bbox + BBOX_MARGIN_MM (adaptive per patient),
vs. fixed FOV_MM centroid crop used in mask_guided.
Everything else identical: mask-normalised T2W/ADC, returns (x, mask, label, pid).
"""
import csv
import os
import random
...
SKIP_PATIENTS  = set()
TARGET_SPACING = 0.5    # resample to this in-plane spacing (mm)
BBOX_MARGIN_MM = 20.0   # physical margin added to each side of gland bbox
BBOX_MIN_HALF  = 30.0   # minimum half-side of crop (mm) — ensures ≥60mm FOV


def _resample_inplane(arr_hwz, current_spacing):
    """Resample H×W in-plane to TARGET_SPACING; keep Z unchanged."""
...
def _gland_bbox_2d(gland_3d):
    """
    Project gland mask across all slices → 2D bbox.
    Returns (cy, cx, half_px):
      cy, cx   — center of bbox in pixel coords
      half_px  — half-side of square crop (pixels)
                 = max(bbox_half + margin, BBOX_MIN_HALF) at TARGET_SPACING
    Fallback to image center + BBOX_MIN_HALF when gland is empty.
    """
    ...
    if len(ys) == 0:
        ...
        return H / 2.0, W / 2.0, BBOX_MIN_HALF / TARGET_SPACING
    ...
    margin_px   = BBOX_MARGIN_MM / TARGET_SPACING
    min_half_px = BBOX_MIN_HALF  / TARGET_SPACING
    # square crop: take max of H and W half-extents, then add margin
    half_px = max(
        (y1 - y0) / 2.0 + margin_px,
        (x1 - x0) / 2.0 + margin_px,
        min_half_px,
    )
    return cy, cx, half_px


def _bbox_crop_and_resize(tensor_chw, cy, cx, half_px, target_size=224):
    """Crop square of radius half_px centered at (cy, cx), resize to target_size."""
...
def load_patient(pid, data_root=DATA_ROOT):
    """
    Load T2W, ADC, gland; resample to TARGET_SPACING; mask-normalise;
    compute bbox crop params from gland bounding box.
    """
    ...
    cy, cx, half_px = _gland_bbox_2d(gland)
    cz              = _gland_z_centroid(gland)
    return {'t2w': t2w, 'adc': adc, 'gland': gland,
            'cy': cy, 'cx': cx, 'half_px': half_px, 'cz': cz}
...
def slice_to_tensor(vols, z, target_size=224):
    arr = np.stack([vols['t2w'][:, :, z],
                    vols['adc'][:, :, z],
                    vols['gland'][:, :, z]], axis=0)
    return _bbox_crop_and_resize(torch.from_numpy(arr),
                                 vols['cy'], vols['cx'], vols['half_px'],
...
class MaskGuidedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask = tensor[2::3]  # [32, H, W] — same spatial transform
