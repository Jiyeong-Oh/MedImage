...
class GradCAMPerSlice:
    ...
    Returns one normalized [H,W] map per slice via gradient of class-1 logit
    w.r.t. input channels, averaged over the nch channels of each slice."""
    ...
    def __call__(self, tensor, class_idx=1):
        ...
        grad = x.grad.squeeze(0).cpu()          # [nslices*nch, H, W]
...
def main(args):
    ...
    if args.gradcam:
        ...
        for i, (pid, lbl, prob) in enumerate(zip(pids, test_lbl, test_prob)):
            ...
            tumor_vol = vols['tumor']  # [H, W, n_slices]
