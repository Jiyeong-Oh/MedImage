"""
Training script for PI-CAI csPCa classification.
- Single stratified train/val/test split (70/15/15)
- Depth-as-channel: 32 slices × 3 modalities = 96ch input
- Differential LR: backbone=lr_backbone, head=lr_head
- Optional AdaBoost-style sample reweighting (--boost-beta > 0)
"""
import argparse
import json
import os
...
def boost_update(model, train_ds, weights, device, beta, clip_ratio):
    model.eval()
    new_w      = weights.copy()
    n_wrong    = 0
    aug_state  = train_ds.augment
    train_ds.augment = False

    with torch.no_grad():
        for i in range(len(train_ds)):
            img, label, _ = train_ds[i]
            pred = model(img.unsqueeze(0).to(device)).argmax(1).item()
            if pred != int(label):
                new_w[i] *= np.exp(beta)
                n_wrong  += 1

    train_ds.augment = aug_state
    clip_val = clip_ratio * new_w.mean()
    new_w    = np.clip(new_w, 0, clip_val)
    new_w    = new_w / new_w.sum() * len(new_w)
    return new_w, n_wrong
...
def train_model(train_records, val_records, args, device, nw):
    ...
    if args.boost_beta > 0:
        print(f"  Boosting ON  beta={args.boost_beta}  clip={args.boost_clip}")
    ...
    model = build_model(num_classes=2, pretrained=True, n_slices=args.n_slices, head_depth=args.head_depth, backbone=args.backbone)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.boost_beta > 0:
            weights, n_wrong = boost_update(
                core, train_ds, weights, device, args.boost_beta, args.boost_clip)
            sampler.weights = torch.tensor(weights, dtype=torch.double)
        ...
        boost_str = ''
        if args.boost_beta > 0:
            boost_str = f' | boost_wrong={n_wrong} max_w={weights.max():.1f}'
...
def save_config(args, output_dir):
    tmp = build_model(num_classes=2, pretrained=False, n_slices=args.n_slices, head_depth=args.head_depth, backbone=args.backbone)
    head_layers = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--patience',     type=int,   default=30,
                        help='Early stopping patience (resets after each LR drop)')
    ...
    parser.add_argument('--head-depth',   type=int,   default=2,
                        help='Head MLP layers: 2=1024→512→256→2, 3=→128→2, 4=→64→2')
