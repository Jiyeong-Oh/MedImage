...
def train_model(train_records, val_records, args, device, nw):
    ...
    ci_eff  = counts[0]
    print(f"  Effective train size: {len(train_ds)} (csPCa={cs_eff}, ciPCa={ci_eff})")
    weights = 1.0 / counts[train_labels_arr.astype(int)]
    sampler = None

    if args.cs_oversample > 1:
        # Already balanced via oversampling — use plain shuffle sampler
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if args.boost_beta > 0 and sampler is not None:
        ...
        print(f"  Epoch {epoch:3d}/{args.epochs} | "
              ...
              f"val AUC: {val_auc:.4f} | {lr_str}{boost_str}")
